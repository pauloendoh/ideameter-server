...
export default class AuthService {
  ...
  public async resetPassword(dto: PasswordResetPostDto) {
    if (dto.password !== dto.password2)
      throw new InvalidPayloadError400("Passwords don't match.")

    // Token exists?
    const tokenExists = await this.tokenRepo.passwordResetTokenExists(dto)

    if (!tokenExists)
      throw new NotFoundError404("Token does not exist or it is expired")
